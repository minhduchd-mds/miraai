...
export default function AppV2() {
  ...
  useDialogFocus(historyOpen, '.history-sheet');
  ...
  useEffect(() => {
    try {
      ...
      // Keep in-memory theme when persistence is blocked.
    }
  }, [theme]);
    ...
    <div className={`mira-v2${mira.content ? ' has-result' : ''}`}>
      ...
      <header className="v2-header">
        <div className="v2-brand" aria-label="Mira">
          ...
          <span className="v2-wordmark"><b>Mira</b><small>Voice-first AI Companion</small></span>
        </div>
      ...
      <section className="v2-conversation" aria-label="Cuộc trò chuyện">
        <div className="v2-caption" aria-live="polite" aria-atomic="true">
          <span>{mira.who}</span>
          <p>{mira.caption}{mira.partial && <i className="v2-caret" aria-hidden="true" />}</p>
        </div>
        ...
        <div className="v2-hint">
          <span><kbd>Space</kbd> để nói/ngắt · <kbd>Enter</kbd> để gửi</span>
          <button type="button" className="v2-history-trigger" onClick={() => setHistoryOpen(true)}>Lịch sử · {mira.history.length}</button>
        </div>
      </section>
