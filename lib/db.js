...
export async function ensureSchema(sql) {
  ...
  // Portable continuity snapshot. Stores the latest user-approved Mira Identity Capsule.
  await sql`create table if not exists identity_capsules (
    id bigserial primary key,
  ...
  await sql`create index if not exists identity_capsules_updated_idx
    on identity_capsules (device_id, updated_at desc)`;
