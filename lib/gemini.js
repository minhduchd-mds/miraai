...
// Chat bằng Gemini-flash (free) — cho "bộ não" Mira. system + messages [{role:'user'|'model', text}] → trả text.
// Thử nhiều model flash free, tắt thinking cho 2.5 (kẻo nuốt token). Không key → null (caller báo 503).
export async function generateChat(system, messages, { maxTokens = 400 } = {}) {
  const key = apiKey();
  if (!key) return null;
  const contents = (messages || [])
    .filter((m) => m && m.text)
    .map((m) => ({
      role: m.role === 'model' || m.role === 'assistant' || m.role === 'mira' ? 'model' : 'user',
  ...
  let last = '';
  for (const model of models) {
    const cfg = { temperature: 0.85, maxOutputTokens: maxTokens };
    if (model.startsWith('gemini-2.5')) cfg.thinkingConfig = { thinkingBudget: 0 };
    ...
    if (!r.ok) { last = `${r.status} (${model})`; continue; }
    const j = await r.json();
    const txt = (j?.candidates?.[0]?.content?.parts || []).map((p) => p?.text || '').join('').trim();
    if (!txt) { last = `empty (${model})`; continue; }
    return txt;
  }
  throw new Error(`Gemini chat thất bại mọi model: ${last}`);
}
