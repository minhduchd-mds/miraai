...
export async function generateJson(prompt, { maxTokens = 400 } = {}) {
  ...
  for (const model of models) {
    ...
    if (r.status === 429) { last = `429 (${model})`; continue; } // hết quota model này → thử model khác
    if (!r.ok) throw new Error(`Gemini gen ${r.status} (${model}): ${(await r.text()).slice(0, 160)}`);
    const j = await r.json();
    const txt = (j?.candidates?.[0]?.content?.parts || []).map((p) => p?.text || '').join('').trim();
    try { return JSON.parse(txt); } catch { return null; }
  }
  throw new Error(`Gemini gen hết quota free mọi model (${last})`);
}
